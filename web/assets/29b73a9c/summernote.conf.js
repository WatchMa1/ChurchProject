const summernoteParams = {
	...
	callbacks: {
		onImageUpload: function (files) {
			sendFile(
				files[0],
				summernoteParams.fileField,
				summernoteParams.fileClass
			);
		},
	},
};
...
function sendFile(file, fileField, fileClass) {
	...
	$.ajax({
		...
		success: function (response) {
			...
			const isImage = fileBlock
				.find(".floor12-file-object")
				.hasClass("floor12-file-object-image");
			const filename = fileBlock
				.find(".floor12-file-object")
				.data("filename");
			const title = fileBlock.find(".floor12-file-object").data("title");
			$(
				".floor12-files-widget-list[data-field='" + fileField + "']"
			).append(fileBlock);
			if (isImage) document.execCommand("insertImage", false, filename);
			else document.execCommand("createLink", false, filename.toString());
		},
